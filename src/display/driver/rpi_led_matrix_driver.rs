...
use rpi_led_matrix::{
    LedCanvas as RpiCanvas, LedColor, LedMatrix, LedMatrixOptions, LedRuntimeOptions,
};
use std::any::Any;
use std::fmt::Debug;
...
impl LedDriver for RpiLedMatrixDriver {
        ...
        Self: Sized,
    {
        ...
        debug!(
            "Initializing C++ binding driver with options: {:?}",
            options
        );
...
impl RpiLedMatrixDriver {
    ...
    fn create_matrix_options(
        options: &MatrixOptions,
    ) -> Result<(LedMatrixOptions, LedRuntimeOptions), String> {
    ...
    fn map_multiplexing(multiplex_str: &str) -> u32 {
        match multiplex_str.to_lowercase().as_str() {
            ...
            _ => {
                warn!(
                    "Unknown multiplexing type '{}' for binding driver, using default (Stripe)",
                    multiplex_str
                );
    ...
    fn map_row_setter(row_setter: &str) -> u32 {
        match row_setter.to_lowercase().as_str() {
            ...
            _ => {
                warn!(
                    "Unknown row address setter '{}' for binding driver, using default (Direct)",
                    row_setter
                );
